...
esemInvaTarget <- function(Fs, Data, GroupVar, Groups, Use, FileOut=getwd(), FileIn){
  #WARNINGS
  ...
  #efa syntax line
  efaLine <- paste("F1-","F",Fs, " BY ", names(Data[,Use[1]]),"-", names(Data[,Use[1]]))

  #means
  meanLine <-  paste("[F1-","F",Fs "@", 0, "]", ";", sep="")
  #Variances
  variances <- paste("F1-","F",Fs "@", 1, ";", sep="")
  #covariances
  LatentNames <- paste("F", 1:Fs)
